...
def compile_docx():
    ...
    sub_run = subtitle_p.add_run("อาร์คที่ 1: โลกที่คุ้นแต่ไม่คุ้น (ตอนที่ 1 - 20)\n\n\n\n")
    ...
    for i in range(1, 21):
        ...
        # Parse title from header line: e.g. # ตอนที่ 1: "Zero"
        # Match: # ตอนที่ X: "Title" or # ตอนที่ X: Title
        ...
        for line in story_lines:
            ...
            # Skip sub-headings or formatting lines if any, otherwise treat as paragraph
            p = doc.add_paragraph()
            p.paragraph_format.space_after = Pt(6)
            p.paragraph_format.line_spacing = 1.15
            p.paragraph_format.first_line_indent = Inches(0.5) # Standard paragraph indent
            
            # Clean markdown formatting like **, *
            # Bold check
            clean_line = line_str.replace("**", "").replace("*", "").replace("`", "")
            
            # If line was italic/bold, we can keep it simple or format it
            run = p.add_run(clean_line)
            set_font(run, font_name, 14) # 14pt as requested by the user
            
        # Add page break after each chapter except the last one
        if i < 20:
            doc.add_page_break()
            
    output_path = "/Users/dolphin/Desktop/Novel/Arc1_Protocol_Arceus.docx"
    doc.save(output_path)
    print(f"Successfully compiled Arc 1 to {output_path}")
