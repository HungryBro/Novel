...
def compile_docx():
    ...
    sub_run = subtitle_p.add_run("อาร์คที่ 7: ผู้ถูกเลือกที่ไม่เคยเลือก (ตอนที่ 161 - 180)\n\n\n\n")
    set_font(sub_run, font_name, 18)
    
    doc.add_page_break()
    
    # Loop over written episodes of Arc 7 (161 to 180)
    for i in range(161, 181):
        filename = f"episode-{i}.md"
        ...
        # Parse title from header line: e.g. # ตอนที่ 161: "เมืองแห่งการชุมนุมบนยอดเขา"
    ...
    output_path = "/Users/dolphin/Desktop/Novel/Arc7_Protocol_Arceus.docx"
    doc.save(output_path)
    print(f"Successfully compiled Arc 7 to {output_path}")
